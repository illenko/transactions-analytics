...
type AnalyticMapper interface {
	ToResponse(items []dbmodel.AnalyticItem) model.Analytic
	ToDayResponse(items []dbmodel.DateAnalyticItem) model.Analytic
	ToMonthResponse(items []dbmodel.DateAnalyticItem) model.Analytic
}
...
func (m *analyticMapper) ToResponse(items []dbmodel.AnalyticItem) model.Analytic {
...
func (m *analyticMapper) ToDayResponse(items []dbmodel.DateAnalyticItem) model.Analytic {
	return m.toDatedResponse(items, m.toDayGroup)
}

func (m *analyticMapper) ToMonthResponse(items []dbmodel.DateAnalyticItem) model.Analytic {
	return m.toDatedResponse(items, m.toMonthGroup)
}
func (m *analyticMapper) toDatedResponse(items []dbmodel.DateAnalyticItem, formatter datedFormatter) model.Analytic {
	return model.Analytic{
